...
namespace FU::Draw
{
	...
	FISHYUTILS_API void DrawDebugOrientedActorPrimitiveComponents(const UWorld* World, const AActor* Actor, FColor Color, float Time, float Thickness = 2, uint8 DepthPriority = 0, TArray<UClass*>* IgnoredClasses = nullptr);
	FISHYUTILS_API void DrawDebugOrientedActorPrimitiveComponentsFrame(const UWorld* World, const AActor* Actor, FColor Color, float Thickness = 2, uint8 DepthPriority = 0, TArray<UClass*>* IgnoredClasses = nullptr);
}
